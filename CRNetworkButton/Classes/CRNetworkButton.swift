...
open class CRNetworkButton: UIButton {
    ...
    fileprivate lazy var progressLayer: CAShapeLayer = {
        ...
        layer.path = UIBezierPath(arcCenter: self.boundsCenter, radius: self.boundsCenter.y - self.crLineWidth / 2,
                                  startAngle: CGFloat(-Float.pi / 2), endAngle: 3 * CGFloat(Float.pi / 2), clockwise: true).cgPath
...
extension CRNetworkButton {
    ...
    fileprivate func setupButton() {
        ...
        startTitleColor = titleColor(for: UIControl.State())
    ...
    fileprivate func updateText() {
        ...
        case .finished:
            setTitle(stopedByError ? errorText : endText, for: UIControl.State())
        }
    }
    ...
    fileprivate func prepareLoadingAnimation(_ completion: (()->())?) {
        ...
        if !shouldAutoReverse {
            setTitleColor(crDotColor, for: UIControl.State())
        }
    }
    ...
    fileprivate func startLoadingAnimation() {
        ...
        let lineOffset:CGFloat = CGFloat(2 * Float.pi) / CGFloat(linesCount)
    ...
    fileprivate func dotsScalingAnimation() {
        ...
        let angleOffset = CGFloat(2 * Float.pi) / CGFloat(linesCount)
    ...
    fileprivate func checkMarkAndBoundsAnimation() {
        ...
        updateText()
        if stopedByError {
            setTitleColor(crErrorColor, for: UIControl.State())
        }
        ...
        opacityTitleAnim.fillMode = CAMediaTimingFillMode.both
